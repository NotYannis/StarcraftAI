...
void ExampleAIModule::onStart()
{
	...
	// Hello World!
	Broodwar->sendText("gl & hf! Nan jdèc GET REKT !!");
	// Print the map name.
...
void ExampleAIModule::onFrame()
{
	...
	for (auto &u : Broodwar->self()->getUnits())
	{
		...
		if (u->isBeingConstructed()){
			if (stopProduction && u->getType() == UnitTypes::Protoss_Assimilator){
		...
		{
			Unit refinery = u->getClosestUnit(IsRefinery);
			if (!refinery && Broodwar->self()->minerals() >= UnitTypes::Protoss_Assimilator.mineralPrice())
			{
				stopProduction = true;
				Broodwar->sendText("Searching gas!");
				TilePosition gasTyle = Broodwar->getBuildLocation(UnitTypes::Protoss_Assimilator, u->getTilePosition());

				if (u->build(UnitTypes::Protoss_Assimilator, gasTyle)){
					continue;
				}
			}
			...
			if (u->isIdle())
			{
				if (scouting.scout == nullptr){
					scouting.SetScout(u);
					Broodwar->sendText("Scout found !");
				}
				if (u->getID() == scouting.scout->getID()){
				...
				{                             // is carrying a powerup such as a flag
					...
					if (refinery != NULL && !refinery->isBeingConstructed() && workersOnGas < 2){
						u->gather(refinery);
						workersOnGas++;
					}
		...
		else if (u->getType().isResourceDepot()) // A resource depot is a Command Center, Nexus, or Hatchery
		{
			...
			if (u->isIdle() && !stopProduction && !u->train(u->getType().getRace().getWorker()))
			{
